...
搜索后端如果对较长的解释型查询返回明显低相关结果，cc-web 会用更短、更贴近核心词的查询在同一后端重试一次。例如 `raft consensus algorithm leader election log replication explained` 这类长查询跑偏时，会尝试收敛到 `raft consensus algorithm`；`Redis cluster vs sentinel high availability comparison 2025` 这类比较查询会保留 `vs` 结构并收敛到 `redis cluster vs sentinel`。排序时还会参考查询词覆盖度，让命中更多核心词的结果优先于泛教程或官网页。如果短查询重试后仍然低相关，该后端会被视为本次失败并进入后续 fallback，避免把低质量结果缓存成成功。返回结果会包含 `query_retry` 或 `query_retries` 诊断字段。
