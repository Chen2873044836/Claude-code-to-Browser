...
搜索后端如果对较长的解释型查询返回明显低相关结果，cc-web 会用更短、更贴近核心词的查询在同一后端重试一次。例如 `raft consensus algorithm leader election log replication explained` 这类长查询跑偏时，会尝试收敛到 `raft consensus algorithm`；`Redis cluster vs sentinel high availability comparison 2025` 这类比较查询会保留 `vs` 结构并收敛到 `redis cluster vs sentinel`。返回结果会包含 `query_retry` 或 `query_retries` 诊断字段。
