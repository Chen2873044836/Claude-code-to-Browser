...
def _check_config(path: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    if data is None:
        recommendations.append("Check config.json path or run from the cc-web project directory.")
...
def _check_claude_instructions(path: Path) -> tuple[dict[str, Any], list[str]]:
    recommendations: list[str] = []
    if not path.exists():
        recommendations.append("Run scripts/install_instructions.py to add cc-web routing hints.")
        return {"ok": False, "path": str(path), "error": "file missing"}, recommendations

    text = path.read_text(encoding="utf-8-sig", errors="replace")
    ok = "cc-web" in text.lower() and "WebSearch" in text
    if not ok:
        recommendations.append("Run scripts/install_instructions.py to refresh cc-web routing hints.")
...
def _hook_command_mentions_guard(command: Any) -> bool:
    ...
    return "cc_web_mcp/hooks/guard.py" in normalized or "cc-web-mcp/hooks/guard.py" in normalized or "hooks/guard.py" in normalized
...
def _check_hook_event(settings: dict[str, Any], event_name: str, required_samples: tuple[str, ...] = ()) -> tuple[bool, str]:
    ...
    if not guard_entries:
        return False, f"{event_name} hook does not run hooks/guard.py"
...
def _check_hook_guard(path: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    data, error = _read_json(path)
    if data is None:
        recommendations.append("Run scripts/install_hook.py to add the cc-web hook guard.")
        return {"ok": False, "path": str(path), "error": error}, recommendations
    ...
    if not ok:
        recommendations.append("Run scripts/install_hook.py to refresh the cc-web hook guard.")
    return {
        "ok": ok,
        "path": str(path),
...
def _check_network() -> tuple[dict[str, Any], list[str]]:
    recommendations: list[str] = []
    try:
        if str(ROOT) not in sys.path:
            sys.path.insert(0, str(ROOT))
        from web import check_health

        health = asyncio.run(check_health())
    except Exception as exc:
        recommendations.append("Run health_check inside Claude Code or retry doctor with --skip-network.")
...
def build_report(config_path: Path, claude_memory_path: Path, settings_path: Path, skip_network: bool) -> dict[str, Any]:
    recommendations: list[str] = []
    ...
    if skip_network:
        ...
        network_check, network_recs = _check_network()
    ...
    checks: dict[str, Any] = {
        ...
        "project": {"ok": (ROOT / "server.py").exists(), "path": str(ROOT)},
        "config": config_check,
